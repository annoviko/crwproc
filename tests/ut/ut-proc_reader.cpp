...
TEST(ut_proc_reader, find_value_float_in_heap) {
    ...
    test_template_find_value<filter_equal>(stack_value.get(), [stack_value]() { (*stack_value) += float(0.1); });
}


TEST(ut_proc_reader, find_value_double_in_heap) {
    std::shared_ptr<double> stack_value = std::make_shared<double>(7.105);
    test_template_find_value<filter_equal>(stack_value.get(), [stack_value]() { (*stack_value) += 0.1; });
}
